...
def main():
    ...
    codigos_unidades = {
        ...
        for (key, value) in map(
            lambda x: (x.string, re.search("codcg=(\d+)", x.get("href")).group(1)),
            links_unidades,
        )
    }
    ...
    for unidade, codigo in codigos_unidades.items():
        if int(codigo) in campus_por_unidade:
            campus = campus_por_unidade[int(codigo)]
        else:
            campus = "Outro"
...
async def iterar_unidades(codigos_unidades):
    ...
    session = aiohttp.ClientSession(
        headers={
            "User-Agent": "MatrUSPbot/2.0 (+http://www.github.com/matrusp/matrusp)"
        }
    )
    ...
    logger.info(f" -   {len(materias)} materias encontradas")
    r = await asyncio.gather(*[parsear_materia(num) for num in materias])
...
async def iterar_unidade(codigo):
    ...
    response = await session.get(
        "https://uspdigital.usp.br/jupiterweb/jupDisciplinaLista?letra=A-Z&tipo=T&codcg="
        + codigo,
        timeout=120,
    )
...
async def parsear_materia(materia):
    ...
    async with semaforo:  # Semaforo controla o número de chamadas simultâneas
        logger.info(f" -      Obtendo turmas de {materia[0]} - {materia[1]}")
        codigo = materia[0]
        try:
            response = await session.get(
                "https://uspdigital.usp.br/jupiterweb/obterTurma?print=true&sgldis="
                + codigo,
            ...
            try:
                logger.warn(
                    f" -      O pedido de turmas de {codigo} excedeu o tempo limite do pedido. Tentando novamente..."
                )
                response = await session.get(
                    "https://uspdigital.usp.br/jupiterweb/obterTurma?print=true&sgldis="
                    + codigo,
                ...
                logger.error(
                    f" -      O pedido de turmas de {codigo} excedeu o tempo limite do pedido"
                )
                return
        except:
            logger.exception(
                f" -      Não foi possível obter turmas de {materia[0]} - {materia[1]}"
            )
            return

        logger.info(f" -      Analisando turmas de {materia[0]} - {materia[1]}")
        ...
        try:
            ...
            logger.exception(
                f" -     Não foi possível parsear turmas de {materia[0]} - {materia[1]}"
            )
            return

        if not turmas:
            logger.warning(
                f" -      Disciplina {codigo} não possui turmas válidas cadastradas no Jupiter. Ignorando..."
            )
            return

        logger.info(f" -      Obtendo informações de {materia[0]} - {materia[1]}")
        try:
            response2 = await session.get(
                "https://uspdigital.usp.br/jupiterweb/obterDisciplina?print=true&sgldis="
                + codigo,
                timeout=args.timeout,
                ssl=False,
            )
            ...
            try:
                logger.warn(
                    f" -      O pedido de informações de {codigo} excedeu o tempo limite do pedido. Tentando novamente..."
                )
                response2 = await session.get(
                    "https://uspdigital.usp.br/jupiterweb/obterDisciplina?print=true&sgldis="
                    + codigo,
                ...
                logger.error(
                    f" -      O pedido de informações de {codigo} excedeu o tempo limite do pedido"
                )
                return
        except:
        ...
        tabelas_folha = soup.find_all(eh_tabela_folha)
        try:
            materia = parsear_info_materia(
                tabelas_folha
            )  # Obter informações da matéria
        except:
            logger.exception(
                f" -     Não foi possível parsear informações de {materia[0]} - {materia[1]}"
            )
            return

        if not materia:
            logger.warning(
                f" -      Disciplina {codigo} não possui informações cadastradas no Jupiter. Ignorando..."
            )
            return
...
def parsear_info_materia(tabelas_folha):
    ...
    for folha in tabelas_folha:
        ...
        if folha.find(string=re_nome):  # Cabeçalho
            ...
            info["campus"] = campus_por_unidade.get(
                int(codigos_unidades[info["unidade"]]), "Outro"
            )  # Obter o campus a partir da unidade
            search = re.search("Disciplina:\s+([A-Z0-9\s]{7})\s-\s(.+)", strings[2])
            assert (
                search != None
            ), f"{strings[2]} não é um nome de disciplina válido ({folha})"
            ...
            "".join(trs[0].stripped_strings) == "Programa Resumido"
        ):  # Programa Reduzido
            info["programa_resumido"] = "".join(trs[1].stripped_strings)
        elif folha.find(string=re_creditos):
            info.update(
                parsear_creditos(folha)
            )  # Adicionar os créditos às informações obtidas
    return info
...
def parsear_turmas(tabelas_folha):
    ...
    for folha in tabelas_folha:
        if folha.find_all(string=re.compile("Código\s+da\s+Turma", flags=re.UNICODE)):
            if info != None:
                if not horario:
                    logger.warn(
                        f" -      Turma {info['codigo']} não possui horário cadastrado"
                    )
                elif not vagas:
                    logger.warn(
                        f" -      Turma {info['codigo']} não possui vagas cadastradas"
                    )
                else:
            ...
            string=re.compile("Atividades\s+Didáticas", flags=re.UNICODE)
        ):
            continue
    ...
    try:
        for tr in tabela.find_all("tr"):
            ...
            if re.search("Código\s+da\s+Turma\s+Teórica", tds[0], flags=re.U):
                ...
                info["inicio"] = dateutil.parser.parse(tds[1], dayfirst=True).strftime(
                    "%d/%m/%Y"
                )
            elif re.search("Fim", tds[0], flags=re.U):
                info["fim"] = dateutil.parser.parse(tds[1], dayfirst=True).strftime(
                    "%d/%m/%Y"
                )
...
def parsear_vagas(tabela):
    ...
    for tr in tabela.find_all("tr"):
        ...
        if len(tds) == 5 and tds[0] == "":  # Cabecalho
            ...
            len(tds) == 5 and tds[0] != ""
        ):  # Novo tipo de vaga (Obrigatória, Optativa, ...)
            if accum != None:
                vagas[tipo] = accum
            tipo = tds[0]
            accum = {
            ...
            len(tds) == 6
        ):  # Detalhamento das vagas (IME - Matemática Bacharelado, Qualquer Unidade da
            # USP, ...)
            grupo = tds[1]
            detalhamento = {
                "vagas": to_int(tds[2]),
...
def parsear_horario(tabela):
    ...
    #        print horario
    return list(
        map(lambda x: dict(zip(("dia", "inicio", "fim", "professores"), x)), horario)
    )
