...
CREATE OR REPLACE VIEW `ListaMedias` AS select `v`.`APid` AS `APid`,avg(`v`.`nota`) AS `AVG(nota)`,count(0) AS `COUNT(*)` from `votos` `v` where (`v`.`tipo` <> 5) group by `v`.`APid`;
